...
$\ttup{r}$, and hence cover the logics $\cMLRK$, $\cMLRKE$,
$\cMLRKM$ and $\cMLRKME$.  We will show in
Section~\ref{expressivity} that $\cMLS$ has the same expressive
power than $\HL(\down)$ and hence we don't need to introduce a new
notion of model equivalence for this logic, we can use hybrid
...
and we will write $\equiv^{\EF}_\gL$ when we need to specify the
bisimulation associated to the logic $\gL$).
\end{defn}
...
For the logics including the operators
$\{\tup{r},\remember,\known\}$ the \emph{bisimulation} should
...
context, and we will write $\bisim_\gL$ when we need to specify the
bisimulation associated to the logic $\gL$).
\end{defn}

It is not difficult to prove that the notions of
...
$\tup{\model_1,w_1} \equiv_\mathcal{L} \tup{\model_2,w_2}$ if for
any formula $\varphi$ in the language of the logic $\mathcal{L}_i$
