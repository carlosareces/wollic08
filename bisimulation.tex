...
\tb{better presented as a definition, do we need to include the game for erase also?}
\tb{agreening states is not well defined.  And perhaps we should move it to the definition of semantics}
...
\tb{The best notation in my oppinion is using pointed model $\tup{\gM,w} \bisim_g \tup{\gM', w'}$ if there exists a game, and $\tup{\gM,w} \bisim_s \tup{\gM', w'}$
if there exist a bisimulation relation.  In any case, if we will discuss also the
case of bisimulation for $\erase$ we will need to indicate which bisimulation we
are talking about.}
...
\tb{change to definition.  Also $\bar{m}$ is used for a tuple of
elements not for a set.  I would use just $M$.}
