...
\models \varphi$ iff $\model \models
\Tr(\varphi)$.
\smallskip
...
We claim $\tup{\model_1,w} \equiv^\EF_{\cMLRKM} \tup{\model_2,w}$. As
every state in both models has a unique successor, Duplicator has
only one way of playing, which is actually a winning
strategy. Hence $\tup{\model_1,w} \equiv_{\cMLRKM} \tup{\model_2,w}$. But
 $\model_1,w \not \models
\diam{r}\remember\diam{r}\diam{r}\known$, while $\model_2, w \models \diam{r}\remember\diam{r}\diam{r}\known$.
\end{pf}
...
\end{thm}
\begin{pf}
We will prove that there is a property $\varphi$ expressible in $\hlogic$
that cannot be expressed in $\cMLRKE$ over finite models.
...
$\tup{\gM^1_n,w_0}\equiv^{\EF^n}_{\cMLRKE}\tup{\gM^2_n,w_0}$. This implies
that there is no $\cMLRKE$-formula $\psi$ capable of expressing this
property.

Let $\varphi =
\down i. \diam{r}(i \land \diam{r}(\lnot i \land \down i.
\diam{r}i))$ as in the proof of Theorem~\ref{thm:tle_leq_hlogic},
and let, for $n\geq 1$, $\model^1_n = \diam{W_n, R_1, V_1,
\emptyset}$ and $\model^2_n = \diam{W_n, R_2, V_2, \emptyset}$
where
$W_n =\{w_0, \dots, w_{n+1}\}$,
$R_1 =\{(n,m)\mid n \neq m\} \cup \{(w_0,w_0)\}$,
$R_2 = R_1 \cup \{(w_1, w_1)\}$, and $V_1(p)=V_2(p)=\emptyset$ for all $p \in \prop$
($\gM^1_2$ and $\gM^2_2$ are shown in Figure~\ref{M1yM2}).
