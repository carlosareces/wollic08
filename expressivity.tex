...
by the $\cMLRKM$-formula $\remember \diam{r}\known$.
%
%\begin{figure}
%\caption{$\cMLRKM\not=\bml$.}
%\label{fig:L3K}
...
\tb{C: we won't say anyting about $\cMLRKME$ is that ok?}


\subsection{Memory Logics and Hybrid Logics}

We will now compare the expressive power of memory logics with
respecto to hybrid logics.  The most natural choice for the
...
\begin{pf}
We first prover that $\cMLRKE \le \hlogic$. We define
...
use infinite models.  Actually, $\cMLRKE < \hlogic$ even on finite models.
...
$\Tr(\varphi)$ has at most $n$ remember operators, we know that
\[
...
surprize, as we could think that the restricted access to the elements
in the stack might actually restrict the expressive power of $\cMLS$.
